...
# metti anche dei file dove appendere l'output di errore per ogni programma (usa l'append mode quindi >>)
...
do
	...
	freebayes -f ${file_dir}/universe.fasta -m 20 -C 5 -Q 10 --min-coverage 10 ../bam/${ar_trio}_mother.bam ../bam/${ar_trio}_child.bam ../bam/${ar_trio}_father.bam  > ${ar_trio}.vcf
 	...
 	# filtration
        grep "#" ${ar_trio}.vcf > candilist${ar_trio}.vcf
	cat ${ar_trio}.vcf| grep -v "##" | python /home/BCG_2024_mcominelli/try "-ar" "high" >> candilist${ar_trio}.vcf
	
	# sorting of family member names
	bcftools query -l candilist${ar_trio}.vcf | sort > samples.txt # extract samples, sort names, save to file
        bcftools view -S samples.txt candilist${ar_trio}.vcf > ${ar_trio}.sorted.vcf # print samples in “sorted” order to new file

        # bedtools
        bedtools intersect -a ${ar_trio}.sorted.vcf -b ${file_dir}/targetsPad100.bed -u > ${ar_trio}_candilistTG.vcf
